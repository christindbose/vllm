...
def generate_block_token_ids(block_id, block_size, max_token_id=100000):
    # NON-OVERLAPPING block-id -> token-id mapping. Each hash_id deterministically
    # maps to its own contiguous, disjoint token range [base, base+block_size).
    # The original implementation used range(block_id, block_id+block_size), which
    # produced heavily overlapping blocks (block 0 = [0..511], block 1 = [1..512], …)
    # — that pattern of mostly-repeated low-ID tokens wedged the FlashInfer engine
    # under serving on this fork. Disjoint ranges sidestep the issue while
    # preserving the only property that matters: same hash_id -> same tokens.
    cycle = max(max_token_id - block_size, 1)
    base = (block_id * block_size) % cycle
    return list(range(base, base + block_size))
